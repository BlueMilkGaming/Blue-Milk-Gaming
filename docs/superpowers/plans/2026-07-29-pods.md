...
- **Spec deviations, all deliberate:**
  ...
  - A result reported on the last match of round 3 closes the pod immediately, so that one result has no in-product flag window: `resolveFlag` requires an existing `flaggedBy` and throws for a match that was never flagged, so a post-close dispute on it has no in-product path and needs a manual ledger/balance adjustment by the admin. Prior-round flags during play are no longer confined to the current round: `/play` now surfaces the flag button on the player's own matches from earlier rounds too, so the gap is narrowed to that one closing match, matching the spec's "flags freeze payouts **until the pod closes**".
