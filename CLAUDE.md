...
**Seat liveness shipped (2026-08-02):** the /play poll doubles as a seat heartbeat (`lastSeenAt`, throttled to one write per minute per seat, filling pods only); `tablesSnapshot` stands up seats quiet for 5 minutes through the shared `removeSeat` path (also serving player leave and admin kick), so a closed tab frees its chair and an AWOL host passes seat 1; lobby TTL cut to 30 minutes; `/admin/pods` (admin nav) has per-seat kick buttons and a 14-day pod history. Verified in production 2026-08-02: sim seat went quiet, an anonymous poll swept it, the emptied pod flipped to abandoned and the sim's `activePodId` cleared; admin gate 404s signed-out; test data cleaned up. Signed-in spot check same day confirmed /admin/pods rendering and the heartbeat's live "seen" times (kick shares the sweep-proven `removeSeat` path). Spec: `docs/superpowers/specs/2026-08-02-pod-seat-liveness-design.md`.
...
- **Dev and build no longer share a directory — keep it that way.** `next dev` and `next build` both default to `.next` with no locking, and a dev server running during a deploy's build corrupted the deployed bundle (prod outage 2026-07-30: every SSR route 500ed with `MODULE_NOT_FOUND` on a turbopack chunk). Fixed in `next.config.ts`: dev writes to `.next-dev`, builds and deploys use `.next`. If you ever see missing-chunk errors in the server Lambda right after a deploy, suspect this class of corruption: stop any dev server, `rm -rf .next`, redeploy.
- **A stale browser tab wedges `next dev`.** If a tab holding a page from a dead dev-server instance reconnects to a new one, Next 15.5 (turbopack) spirals into `_buildManifest.js.tmp` ENOENT errors and every route 500s, even after deleting the build directory. The fix is the order: stop the server, delete `.next-dev`, start it, then hard-reload the tab before anything else polls. Symptom to recognize: curl says 200 while the browser says Internal Server Error, then everything 500s.
